...
function Check() {
    ...
    const handleSchemaChange = (e) => {
      const selectedOption = availableSchemas.find(schema => schema.value === e.target.value);
      if (selectedOption) {
        const newSchema = {
        ...
        setAddedSchemas([...addedSchemas, newSchema]);
        const updatedAvailableSchemas = availableSchemas.filter(
          (schema) => schema.value !== selectedOption.value
        );
        setAvailableSchemas(updatedAvailableSchemas);
        setSelectedSchema("");
      }
